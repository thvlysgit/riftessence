...
- `/games/archive`: six guesses using role, resource, attack range and skin-count clues (excluding base appearances and chromas). Each row displays the guessed champion’s own skin count; the arrow compares it to the answer. Inspired by [LoLdle](https://loldle.net/), credited in-game. Each guess after the first reduces the offered reward by 10 PE.
...
pnpm exec jest --runInBand --config jest.config.cjs --runTestsByPath apps/api/__tests__/dailyGames.test.ts apps/api/__tests__/itemPriceGame.test.ts apps/api/__tests__/economy.integration.test.ts
```
