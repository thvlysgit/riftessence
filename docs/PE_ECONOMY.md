...
- `/games/recipe-rush`: three final items (two-component epic, two-component legendary, then a legendary with at least three ingredients). Some legendary rounds require crafting their epic components first, including nested epic recipes, before final assembly. Drag a piece onto the forge, tap it, or focus it and press Enter/Space. Each action adds exactly one copy. New rounds have ten unique tray items that replenish after use; duplicate ingredients require deliberate repeated uses of the same button. Decoys are restricted to the actual ingredient tiers for each step: basic-only recipes get basic alternatives, epic-only recipes get epics, and mixed recipes get five choices from each tier. Tear and Dark Seal count as basic recipe ingredients; non-upgrading starter items are excluded. Within those limits, four decoy slots prioritize related stats, shared build ingredients, and similar prices; the rest vary. Recipes and crafting progress are pinned with the round. Untouched trays from older replenishing-tray rounds refresh on resume or the next action when their catalog patch still matches. Existing item keys keep their identity so stale clicks cannot select replacement items. Started steps and original non-replenishing rounds keep their previous trays, progress, and penalties.
