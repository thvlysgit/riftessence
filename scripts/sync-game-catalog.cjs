...
async function main() {
  ...
  for (const c of Object.values(data)) {
    const skinResponse = await fetch(`https://ddragon.leagueoflegends.com/cdn/${version}/data/en_US/champion/${c.id}.json`);
    if (!skinResponse.ok) throw new Error(`Data Dragon skins ${c.id}: ${skinResponse.status}`);
    const skinData = await skinResponse.json();
    const skinCount = skinData.data[c.id].skins.filter(skin => skin.num !== 0).length;
    let abilities = [];
    ...
    champions.push({ id: c.id, name: c.name, title: c.title, roles: c.tags, resource: c.partype, range: c.stats.attackrange >= 300 ? 'Ranged' : 'Melee', skinCount, abilities });
  }
