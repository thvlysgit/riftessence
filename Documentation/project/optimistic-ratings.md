...
- A submitted rating is stored in `PendingRating`, separate from `Rating`. It cannot affect public feedback, scores, rewards, or notifications while pending.
- After ownership succeeds, shared games are checked against the recipient's verified Riot accounts (last 50 matches). Publication rechecks account ownership, self-rating, blocks, duplicates, daily limits, and cooldowns.
...
1. Apply `prisma migrate deploy`, including `20260822110000_add_riot_verification_attempts` and `20260904120000_pending_ratings`.
